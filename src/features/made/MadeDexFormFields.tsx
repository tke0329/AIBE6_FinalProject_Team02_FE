...
export function MadeDexBasicFields({
  name,
  description,
  onNameChange,
  onDescriptionChange,
  ...
  return (
    <>
      <label
        htmlFor="made-dex-name"
        className="block text-sm font-bold text-content-primary">
        도감 이름
      </label>
      <input
        id="made-dex-name"
      ...
      <label
        htmlFor="made-dex-description"
        className="mt-6 block text-sm font-bold text-content-primary">
        소개말
        <span className="ml-1 font-medium text-content-muted">(선택)</span>
      </label>
      <textarea
...
export function MadeDexVisibilityPicker({
  visibility,
  onChange,
      ...
      {VISIBILITY_OPTIONS.map(({ value, label, description, Icon }) => (
        <button
          ...
          className={`flex w-full items-center gap-4 rounded-2xl p-4 text-left shadow-card ${
            visibility === value
              ? 'bg-surface-accent ring-2 ring-orange-400'
              : 'bg-surface-card'
          }`}>
          <span className="min-w-0 flex-1">
            <span className="block font-display text-lg text-content-primary">
              {label}
            </span>
            <span className="mt-1 block text-xs leading-4 text-content-secondary">
              {description}
            </span>
          </span>
