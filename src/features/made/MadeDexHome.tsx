...
interface Props {
    dexId: MadeDexId
    recentCard?: MadeCard | null
    participants: MadeParticipant[]
    onBack: () => void
    onSwitchDex: (id: MadeDexId) => void
    onRegister: () => void
...
const CARDS: MadeCard[] = [
    ...
    { name: '을지로 노포', emoji: '🍲', by: '윤', location: '을지로', tags: ['점심', '혼밥'] },
    { name: '성수 브런치', emoji: '🥞', by: '신', location: '성수동', tags: ['데이트', '주말'] },
    { name: '망원 타코', emoji: '🌮', by: '윤', location: '망원동', tags: ['저녁'] },
    { name: '한남 스시', emoji: '🍣', by: '신', location: '한남동', tags: ['기념일', '재방문'] },
    { name: '연희 베이커리', emoji: '🥐', by: '윤', location: '연희동', tags: ['브런치'] },
    { name: '홍대 라멘', emoji: '🍜', by: '신', location: '홍대', tags: ['혼밥', '점심'] },
    { name: '이태원 커리', emoji: '🍛', by: '윤', location: '이태원', tags: ['저녁'] },
]

// TODO(CATCHEAT-29): 도감 자유 기록 이슈에서 API 응답으로 교체
const DEX_INFO: Record<MadeDexId, { title: string; count: string; emoji: string }> = {
    1: { title: '우리의 데이트 도감', count: '함께 등록한 카드 8개', emoji: '💑' },
    2: { title: '회사 점심 도감', count: '함께 등록한 카드 13개', emoji: '🍱' },
}
        ...
        <div className="relative flex h-full flex-col bg-cream-100">
            <header className="shrink-0 px-4 pt-4">
                <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={onOpenInfo} aria-label="도감 정보" className="min-h-touch shrink-0">
                ...
                <div className="mt-3 flex items-center justify-between rounded-2xl bg-surface-card p-4 shadow-card">
                    <div>
                        ...
                        <div className="mt-2 flex -space-x-2">
                            {participants.map((person) => (
                                <Badge key={person.id} variant="member" label={`참여자 ${person.name}`}>
                                    {person.name}
                                </Badge>
                            ))}
                        </div>
                    ...
                    >
                        <UserPlusIcon size={15} aria-hidden />
                        참여자 추가
                    </button>
                </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10">
                        {results.map((card) => (
                            <FoodCard
                                ...
                                footer={
                                    <p className="truncate text-center text-xs text-content-link">#{card.tags[0]}</p>
                                }
                                overlay={
                                    <Badge
                                        variant="member"
