        ...
        <div className="flex h-full flex-col bg-surface-app">
            <main className="no-scrollbar flex-1 overflow-y-auto">
                <header className="flex items-center gap-3 px-5 pt-4">
                    ...
                    <span className="font-display text-lg text-neutral-900">프로필</span>
                </header>
                <div className="px-5 pt-3">
                    <div className="flex items-center gap-4">
                        <span className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-watermelon-200 font-display text-2xl text-watermelon-700">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-display text-lg text-neutral-900">{user.nickname}</span>
                        </div>
                    </div>
                    <RelationButton
                ...
                {tab === '기본도감' && (
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(104px,1fr))] gap-3 px-5 py-5">
                        ...
                        {basicDex.filter((d) => d.unlocked).length === 0 && (
                            <p className="col-span-full py-10 text-center text-sm text-neutral-800">
                                수집한 도감이 없어요
                            </p>
                        )}
                    </div>
                        ...
                        <div className="space-y-3 px-5 py-3">
                            {challenges.map((c) => (
                                <article key={c.id} className="rounded-2xl bg-white p-4 shadow-soft">
                                    <p className="font-display text-base text-neutral-900">{c.name}</p>
                                    <p className="mt-1 text-xs text-neutral-400">{c.participantCount}명 참여</p>
                                    {c.totalSlots > 0 && (
                                        <div className="mt-2">
...
function RelationButton({
    ...
    if (status === 'FRIEND')
        return (
            <button
                ...
                className={`${base} border-neutral-200 bg-white text-neutral-800`}
            >
    ...
    if (status === 'REQUEST_SENT')
        return (
            <button onClick={handleCancel} className={`${base} border-neutral-200 bg-white text-neutral-800`}>
                <XIcon size={16} /> 요청 취소
            </button>
        )
    if (status === 'REQUEST_RECEIVED')
        return (
            <button onClick={handleReceived} className={`${base} border-watermelon-400 text-watermelon-600`}>
    ...
    return (
        <button
            ...
            className={`${base} border-watermelon-400 text-watermelon-600`}
        >
