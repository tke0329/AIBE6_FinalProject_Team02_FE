                ...
                {requests.map((r) => (
                  <li key={r.id} className="rounded-2xl bg-white p-4 shadow-soft">
                    <div className="flex gap-3">
                      {r.evidenceUrl && (
                        <Image src={r.evidenceUrl} alt="증빙 사진" width={64} height={64} unoptimized
                               className="h-16 w-16 shrink-0 rounded-xl object-cover" />
                      )}
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-brown">{r.description}</p>
                        {r.failureReason && <p className="mt-0.5 text-xs text-brown-soft">AI: {r.failureReason}</p>}
                    ...
                    <div className="mt-3 flex gap-2">
                      <PrimaryBtn onClick={() => run(
                        ...
                        () => setRequests((p) => p.filter((x) => x.id !== r.id)))}>등록 완료</PrimaryBtn>
                      <GhostBtn onClick={() => openReject({ kind: 'request', id: r.id })}>반려</GhostBtn>
                    </div>
                  </li>
                ))}
              </ul>
                  ...
                  <li key={r.id} className="rounded-2xl bg-white p-4 shadow-soft">
                    ...
                    {isPendingReports ? (
                      <div className="mt-3 flex gap-2">
                        <PrimaryBtn onClick={() => run(
                          () => acceptReport(r.id),
                          () => setReports((p) => p.filter((x) => x.id !== r.id)))}>채택</PrimaryBtn>
                        <GhostBtn onClick={() => openReject({ kind: 'report', id: r.id })}>반려</GhostBtn>
                      </div>
                    ) : (
...
function PrimaryBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick}
      className="min-h-touch flex-1 rounded-xl bg-orange-500 text-sm font-medium text-white">{children}</button>
  );
}
...
function GhostBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick}
      className="min-h-touch flex-1 rounded-xl bg-cream-200 text-sm font-medium text-brown-soft">{children}</button>
  );
}
