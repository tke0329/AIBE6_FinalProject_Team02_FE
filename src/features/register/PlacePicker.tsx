...
export function PlacePicker({ value, onChange }: Props) {
    ...
    return (
      <div className="mt-1.5 flex items-center gap-2 rounded-2xl border border-edge-active bg-surface-card px-4 py-3">
        <MapPinIcon
          size={18}
          aria-hidden
          className="shrink-0 text-content-link"
        />
  ...
  return (
    <div className="mt-1.5">
      ...
      {nearbyCoords.supported && (
        <div className="mt-1.5 flex items-center gap-2 px-1">
          <button
            ...
            disabled={
              nearbyCoords.status === "asking" ||
              nearbyCoords.status === "denied"
            }
            aria-label={
              nearbyCoords.status === "unavailable"
                ? "내 주변 — 다시 시도"
                : undefined
            }
          ...
          <span
            aria-live="polite"
            className="min-w-0 flex-1 text-xs text-content-secondary"
          >
            {nearbyCoords.status === "asking" && "위치 확인 중…"}
            {nearbyCoords.status === "denied" &&
              "위치 권한이 막혀 있어 전국에서 찾고 있어요"}
            {nearbyCoords.status === "unavailable" &&
              "위치를 못 찾았어요. 다시 눌러 보세요!"}
      ...
      {searching && (
        <p className="mt-1.5 px-1 text-xs text-content-secondary">찾는 중…</p>
      )}
      ...
      {open && (
        <>
            ...
            {results.map((place, index) => (
              ...
              >
                ...
                <span className="text-xs text-content-secondary">
                  {[place.category, displayAddress(place)]
                    .filter(Boolean)
                    .join(" · ")}
                </span>
              </li>
          ...
          {results.length > 0 && (
            <p className="mt-1 px-1 text-right text-[11px] text-content-muted">
              장소 정보 © Kakao
            </p>
          )}
        </>
