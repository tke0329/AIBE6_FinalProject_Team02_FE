...
export type NearbyStatus = 'idle' | 'asking' | 'granted' | 'denied' | 'unavailable'
...
export function useNearbyCoords() {
    ...
    const [status, setStatus] = useState<NearbyStatus>(cachedCoords ? 'granted' : cachedDenied ? 'denied' : 'idle')
