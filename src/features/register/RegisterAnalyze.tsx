...
import { RETRY_LIMIT_EXCEEDED, SlotVerdict, VerificationResult, verifyFoods } from './verificationApi'
...
export function RegisterAnalyze({ onBack, onProceed }: Props) {
    const { selectedSlots, uploadedPhotoKeys, analysisPhotoIndex, registrationId, setRegistrationId, setRecordSlots } =
        useRegisterFlow()
    ...
    }, [registrationId, uploadedPhotoKeys, analysisPhotoIndex, selectedSlots, setRegistrationId])
    ...
    return (
        ...
        <div className="flex h-full flex-col break-keep bg-surface-app">
            ...
            {phase === 'exhausted' && (
                <Centered
                    icon={<AlertTriangleIcon size={40} aria-hidden className="text-content-link" />}
                ...
                <>
                    <main className="no-scrollbar flex-1 overflow-y-auto px-5 pb-4">
                        ...
                        {failed.length > 0 && !exhausted && (
                            <p className="mt-4 rounded-2xl bg-surface-accent p-3 text-xs leading-5 text-content-secondary">
                                사진을 바꾸거나 음식을 다시 고르면 한 번 더 확인할 수 있어요. 재분석 기회가{' '}
                                <strong className="text-content-link">{result.retriesLeft}번</strong> 남았어요.
                            </p>
                        )}
                        ...
                        {failed.length > 0 && exhausted && (
                            <p className="mt-4 rounded-2xl bg-surface-accent p-3 text-xs leading-5 text-content-secondary">
                                ...
                                <strong className="text-content-link">확인되지 않은 {failed.length}개</strong>는 사진을
                                증빙으로 검토를 요청하고, 통과되면 도감이 열려요.
                            </p>
                        )}
                    </main>
                    ...
                    <div className="shrink-0 space-y-2 px-5 pb-8 pt-4">
                        ...
                        {(passed.length > 0 || exhausted) && (
                            <button
                                type="button"
                                onClick={() => proceedWith(exhausted ? result.verdicts : passed)}
...
function Verifying({ slow }: { slow: boolean }) {
    return (
        <div className="flex flex-1 flex-col items-center justify-center px-5">
            ...
            <p className="mt-5 font-display text-lg text-content-primary">사진을 확인하고 있어요</p>
            <p className="mt-1 text-sm text-content-secondary">{slow ? '조금만 기다려 주세요…' : '금방 끝나요!'}</p>
        </div>
    )
}
...
function Centered({ icon, title, description, primary, secondary }: CenteredProps) {
    return (
        <div className="flex flex-1 flex-col items-center justify-center px-5 text-center">
            {icon}
            <h1 className="mt-4 font-display text-xl text-content-primary">{title}</h1>
    ...
    return (
        <li
            className={`flex items-start gap-3 rounded-2xl border-2 p-3 ${
                verdict.matched ? 'border-edge-active bg-surface-card' : 'border-edge-default bg-surface-card'
            }`}
        >
            {verdict.matched ? (
                <CheckCircle2Icon size={20} aria-hidden className="mt-0.5 shrink-0 text-feedback-success" />
            ) : (
                <XCircleIcon size={20} aria-hidden className="mt-0.5 shrink-0 text-content-muted" />
            )}
            ...
            <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                    ...
                    <span className="text-xs text-content-secondary">{verdict.category}</span>
                </div>
            ...
            <span className="sr-only">{verdict.matched ? '확인됨' : '확인되지 않음'}</span>
        </li>
    )
}
