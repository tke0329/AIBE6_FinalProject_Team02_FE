...
export function FoodNamePicker({ entries }: Props) {
  const { selectedSlots, addSlot, removeSlot, canAddMore, aliases } =
    useRegisterFlow();
  const [query, setQuery] = useState("");
  const [browseCategory, setBrowseCategory] = useState<FoodCategory | null>(
    null,
  );

  const index = useMemo(
    () => buildDexSearchIndex(entries, aliases),
    [entries, aliases],
  );
  ...
  const browseResults = useMemo(
    () =>
      browseCategory
        ? entries.filter((entry) => entry.category === browseCategory)
        : [],
  ...
  const renderSlotButton = (slot: DexEntry) => {
    ...
    return (
      <button
        ...
        aria-label={
          alreadyPicked ? `${slot.name}, 이미 고름` : `${slot.name} 고르기`
        }
  ...
  return (
    <section className="mt-6" aria-label="음식 이름 고르기">
      <div className="mb-1.5 flex items-center justify-between">
        ...
        >
          음식 이름을 입력해 주세요{" "}
          <span className="text-content-link">(필수)</span>
        </label>
      ...
      {selectedSlots.length > 0 && (
        <ul
          className="no-scrollbar -mx-5 mb-2 flex gap-1.5 overflow-x-auto px-5"
          aria-label="고른 음식"
        >
          {selectedSlots.map((slot) => (
            <li key={slot.id} className="shrink-0">
              {/* 높이를 줄이려고 최소 터치 타깃(44px, globals.css --layout-touch-min)을 의도적으로 벗어난다 */}
              <button
                type="button"
                onClick={() => removeSlot(slot.id)}
                aria-label={`${slot.name} 빼기`}
                className="flex h-8 items-center gap-1 rounded-full bg-action-primary px-2.5 text-sm font-bold text-content-on-action"
              >
                {slot.name}
      ...
      {canAddMore && searching && (
        <div className="mt-2 overflow-hidden rounded-2xl border border-edge-default bg-surface-card">
          {results.length > 0 ? (
            <>
              <p className="px-3 pt-2 text-xs font-medium text-content-secondary">
                도감 검색 결과
              </p>
            ...
            {CATEGORY_META.map((meta) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  onClick={() =>
                    setBrowseCategory(active ? null : meta.category)
                  }
                  /* 고른 음식 칩과 같은 높이. 최소 터치 타깃(44px)을 의도적으로 벗어난다 */
