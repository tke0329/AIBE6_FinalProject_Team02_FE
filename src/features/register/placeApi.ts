 ...
 */
export async function searchPlaces(query: string, coords?: SearchCoords): Promise<PlaceSummary[]> {
...
export async function geocodeAddress(address: string): Promise<GeoPoint> {
    return apiFetch<GeoPoint>(`/api/v1/places/geocode?address=${encodeURIComponent(address)}`)
}
