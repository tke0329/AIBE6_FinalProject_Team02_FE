 ...
 */
export async function uploadImageToS3(blob: Blob, fileName: string): Promise<{ key: string; publicUrl: string }> {
    const contentType = blob.type || 'image/jpeg'

    // presigned URL 발급 (이름·타입만 전송)
    const { uploads } = await apiFetch<PresignedResponse>('/api/v1/uploads/presigned', {
        method: 'POST',
    ...
    if (!res.ok) throw new Error('이미지 업로드에 실패했어요. 다시 시도해 주세요.')
